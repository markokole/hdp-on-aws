...
locals {
  ...
  namenodes_ips = slice(
    local.public_ips,
    local.namenode_idx,
    local.namenode_idx + local.no_namenodes,
  )
  namenodes_dns = slice(
    local.public_dns,
    local.namenode_idx,
  ...
  datanodes_ips = slice(
    local.public_ips,
    local.datanode_idx,
    length(local.public_ips),
  )
  datanodes_dns = slice(
    local.public_dns,
    local.datanode_idx,
    length(local.public_dns),
  )
  datanodes_dns_private = slice(
    local.private_dns,
    local.datanode_idx,
    length(local.public_dns),
  )
  ...
  gpfs_quorum = distinct(
    concat(local.namenodes_dns_private, local.datanodes_dns_private),
  ) # for config hdfs-site
...
resource "null_resource" "apply_blueprint" {
  depends_on = [
    null_resource.configure_postgres,
