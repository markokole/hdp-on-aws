...
data "template_file" "ansible_inventory_single" {
  ...
  template = file(
    "${path.module}/resources/templates/ansible_inventory_single.yml.tmpl",
  )
  vars = {
...
data "template_file" "ansible_inventory_cluster" {
  ...
  template = file(
    "${path.module}/resources/templates/ansible_inventory_cluster.yml.tmpl",
  )
  vars = {
    ...
    namenode-ansible-text = join(
      "",
      data.template_file.generate_namenode_hostname_cluster.*.rendered,
    )
    hdp-worker-title = "[hdp-worker]"
    datanode-ansible-text = join(
      "",
      data.template_file.generate_datanode_hostname_cluster.*.rendered,
    )
  }
}
...
resource "local_file" "ansible_inventory_single_render" {
  count    = local.type == "single" ? 1 : 0
  content  = data.template_file.ansible_inventory_single[0].rendered
  filename = "${local.workdir}/ansible-hosts"
}

resource "local_file" "ansible_inventory_cluster_render" {
  count    = local.type == "cluster" ? 1 : 0
  content  = data.template_file.ansible_inventory_cluster[0].rendered
  filename = "${local.workdir}/ansible-hosts"
}
