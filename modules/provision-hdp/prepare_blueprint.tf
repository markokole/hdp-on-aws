...
data "template_file" "dynamic_blueprint" {
  template = file(
    "${path.module}/resources/templates/blueprints/${var.cluster_type}.json.tmpl",
  )
  vars = {
    s3a_access_key  = var.s3a_access_key
    s3a_secret_key  = var.s3a_secret_key
    postgres_server = local.ambari_dns_private
